...
def interactionForce(inputPos , radius , strength , particleIndex): 
    ...
    if sqrDst < radius*radius : 
        ...
        # if dst < epsilon : dirToInput = Vector2()
        # else : dirToInput = offset/dst
...
slider1  = Slider(width-250,5,150,15 , 40)
...
while running : 
    ...
    for i in range(len(particles)) : 
        if pg.mouse.get_pressed()[0] : interactiveForce = interactionForce(inputPos , interactiveForceRadius,-forceStrength , i)
        elif pg.mouse.get_pressed()[2] : interactiveForce = interactionForce(inputPos , interactiveForceRadius, forceStrength , i)
        else : interactiveForce = Vector2()
        netForce = pressForce[i] + viscForce[i]  #+ gravity
        particles[i].acc = netForce / densities[i] + interactiveForce
        particles[i].update()
        # particles[i].vel += interactiveForce*deltaT
